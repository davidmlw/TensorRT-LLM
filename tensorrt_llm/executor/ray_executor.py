...
class RayExecutor(RpcExecutorMixin, GenerationExecutor):
    ...
    def create_workers(self, worker_cls, worker_kwargs):
        ...
        rank = 0
        placement_where = self.placement_where or [self._get_placement_group(tp_size=self.tp_size)]
        self.world_size = sum(len(bundle_indices) for _, bundle_indices in placement_where)
        self.workers = []
        for pg, bundle_indices in placement_where:
