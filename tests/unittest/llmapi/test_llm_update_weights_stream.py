...
def main():
    ...
    device_uuid = fsdp.report_device_id()
    from torch.multiprocessing.reductions import reduce_tensor
    total_available_bytes = 2 * (1024**3)
    cur_available_bytes = total_available_bytes
    converted_params = {}
    cur_handles = []
    stream_step = 0
    ...
    item_count = 0
    handle_count = 0
    for name, param in fsdp.per_tensor_generator():
        size_in_bytes = param.element_size() * param.numel()
        if isinstance(param, DTensor):
            param = param.full_tensor()
        print(f"item_count: {item_count}, handle_count: {handle_count}, name: {name}, {param.shape}, {size_in_bytes}")
        item_count += 1
        gate_up_name = None
        gate_up_pair = None
        ...
        if size_in_bytes > cur_available_bytes:
            print(f"call update_weights_from_ipc_handles, stream_step: {stream_step}")
            stream_step += 1
            ...
            if rank == 0:
                ...
                trtllm.llm.update_weights_from_ipc_handles(all_handles)
    ...
    print(f"item_count: {item_count}, handle_count: {handle_count}")
    if cur_handles:
        print(f"call update_weights_from_ipc_handles tail step, cur_handles: {len(cur_handles)}")
        device_handles = {device_uuid: cur_handles}
